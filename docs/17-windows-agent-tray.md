...
- `agent tray` 已支持 Windows 托盘常驻和本地 HTTP 控制接口，默认离线运行。
- `run --local-only` 已优先投递到托盘，并支持右下角小窗口通过、拒绝、其他回复。
- 本地设置已持久化，支持提醒开关、提醒样式、离线/在线配置、登录信息、开机启动。
- 本地历史已保存会话、输出摘要、审批、决策，并支持保留天数清理和输出捕获模式。
